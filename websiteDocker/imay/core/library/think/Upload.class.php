...
class Upload {
     ...
     */
    public function fetch($file ='', $filename = '') 
    {
        $info    =  array();
        
        if(empty($file)){
            $this->error = '没有上传的URL抓取资源！';
            return false;
        }
        if($filename == ''){
            $this->error = '没有上传的文件名！';
        ...
        if($this->uploader->saveFetch($file, $filename)){
            $info = $filename;
        } else {
    ...
    /**
     * 七牛七牛 视频采样缩略图 
     * 视频采样缩略图接口用于从视频文件中截取多帧画面并按指定大小缩放成图片
     */
    public function vsample($vedio_url, $t, $w_h, $time, $pattern)
    {
        $info    =  array();
        $row = array();
    
        if(empty($vedio_url)){
            $this->error = '没有上传的视频URL资源！';
            return false;
        }
        $info = $this->uploader->getVsample($vedio_url, $t, $w_h, $time, $pattern);
        ...
        if($info){
            ...
            while ($i <= 50){
                if ($i == 50) usleep(600);
                $result = $this->uploader->getVsamplePrefop($info['persistentId']);
                $result = json_decode($result, true);
                if($result['code'] == 0){
                    $row = $result['items'][0]['keys'];
                    break;
                }elseif ($result['code'] == 1 || $result['code'] == 2){
                    continue;
                }else{
                    break;
                }
