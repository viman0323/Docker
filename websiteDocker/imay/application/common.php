...
{
    return widget('common/Ad/run', ['name' => $name]);
}
...
function get_file($file_id, $field = NULL)
{
    ...
    $file = db('File')->where(['id' => $file_id])->find();
    if ($field == 'path') {
...
function get_username($uid = 0)
{
    ...
    $name = db('member')->where(['uid' => $uid])->value('username');
...
{
    $size = in_array($size, ['big', 'middle', 'small', 'real']) ? $size : 'middle';
...
function get_grid_list($list_grids)
{
    ...
    $data = ['grids' => $grids, 'fields' => $fields];
...
function get_document_model($id = NULL, $field = NULL)
{
    ...
    /* 获取模型名称 */
    if (empty($list)) {
        $map   = ['status' => 1, 'is_bind_category' => 1];
...
{
    $str  = md5($str);
    $mdv1 = substr($str, 0, 16);
    $mdv2 = substr($str, 16, 16);
    $crc1 = abs(crc32($mdv1));
    $crc2 = abs(crc32($mdv2));
    
    return number_format(round($crc1 * $crc2), 0, ',', '');
}

function getRetrunStr($rs)
{
    /**
     * 有返回内容且errcode为empty就是请求成功了
     */
    if ($rs !== FALSE && empty($rs['errorcode'])) {
        $ret = [
            'status' => 1,
        ];
    } else {
        $ret = [
            'status'    => -1,
            'errorcode' => !isset($rs['errorcode']) ? "" : $rs['errorcode'],
            'errordesc' => !isset($rs['errordesc']) ? "" : $rs['errordesc'],
        ];
    }
...
/**
 * @brief  把时间段转为为x小时、x分、x秒的形式
 * @author wojiaojason@gmail.com
 */
if (!function_exists('timeTransform')) {
